...
public class DoMonsterActions extends ThearsmonstersDefaultAction {
    ...
    public ActionForward doExecuteGameAction(ActionMapping mapping,
        ...
        MonsterActionsToDo actionsToDo = monsterActionsToDoForm.getActionsToDo();
        
		try {
	        Lair lair = SessionManager.getMyLair(request);
	        monsterFacade.executeMonsterActions(lair, actionsToDo);
	        for(LocalizableMessage notification: actionsToDo.getNotificationMessages()) {
	        	FlashMessage.show(request, notification);
	        }
        } catch (Exception e) {
